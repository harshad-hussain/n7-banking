                  ...
                  <div className="flex justify-between items-center pt-2 select-none">
                    ...
                    <button 
                      ...
                      className={`w-9 h-5 rounded-full transition-colors flex items-center p-0.5 ${
                        cardActive ? 'bg-slate-700' : 'bg-blue-600'
                      }`}
                    >
                      <div className={`w-4 h-4 rounded-full bg-white transition-transform ${
                        cardActive ? 'translate-x-0' : 'translate-x-4'
                      }`}></div>
                    </button>
                  </div>
                </div>
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            <div className="glass-panel rounded-2xl p-6 shadow-md">
              <div className="flex justify-between items-center pb-3 border-b border-slate-800 mb-4">
                ...
                <button className="w-6 h-6 rounded-full bg-amber-500/20 text-amber-500 flex items-center justify-center border border-amber-500/30 hover:bg-amber-500/30 transition-colors">
                  <FaPlus size={10} />
                </button>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {goals.map((goal) => (
                  <div 
                    key={goal.id}
                    className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50 flex flex-col justify-between items-center text-center shadow-sm hover:scale-[1.03] transition-all"
                  >
                ...
                <div className="flex-1 flex gap-3 overflow-x-auto py-1 no-scrollbar select-none">
                  {quickContacts.map((contact, idx) => (
                    <div key={idx} className="flex flex-col items-center shrink-0">
                      <div className={`w-9 h-9 rounded-full ${contact.avatarColor} text-white flex items-center justify-center font-bold text-[11px] border border-slate-700/50`}>
                        {contact.initials}
                      </div>
                      <span className="text-[10px] text-slate-400 font-medium mt-1 uppercase tracking-tight">{contact.name}</span>
                    </div>
                  ))}
                </div>
